...
public class LineChartView extends View implements View.OnTouchListener {
    ...
    private int YCoord(float y0) {
        ...
        try {
            ...
            if (y < ylabelBig1.floatValue()) {//小于最小值的情况
                return surface.YPoint - new BigDecimal(y).divide(ylabelBig1, new MathContext(5, RoundingMode.HALF_EVEN)).multiply(yScaleBig).intValue();
            }

            return surface.YPoint - new BigDecimal(y).subtract(ylabelBig1).
                    divide(perlabelBig, new MathContext(5, RoundingMode.HALF_EVEN)).
    ...
    protected void onDraw(Canvas canvas) {
        ...
        surface.XScale = (surface.XLength / (XLabels.length)) + Utils.dp2px(getContext(), 0);
...
//                canvas.drawLine(startX + i * surface.XScale, surface.YPoint, startX + i * surface.XScale, surface.YPoint - surface.YLength, surface.gridPaint);
            ...
            try {
                //x刻度
                if(i == 0 || i == 15 || i == XLabels.length - 1) {
    ...
    private class Surface {
        ...
        public int lineColor = getResources().getColor(R.color.color_FF5754);
